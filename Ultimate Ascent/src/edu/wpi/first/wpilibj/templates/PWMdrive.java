...
public class PWMdrive {
    ...
    protected static final int motorQuadrant[] = {3,1,2,4}; // lays out PWM inputs in the quadrants of a cartesian plane.
    ...
    public double getSpeed(int side) { //get the speed of one, two, or both sides.
        if (side == RIGHT_SIDE) return spc[0].get();
        else if (side == LEFT_SIDE) return spc[1].get();
        else if (side == BOTH_SIDES) return (spc[0].get() + spc[1].get())/2;
        else return 0;
    }
